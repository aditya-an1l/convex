...
class Abs_Translator(ABC):
    ...
    1) Not necessary => is_eng verified by lang
    2)  Is code corpus verified by iskeyword() fn in keyword module
    '''
...
class Translator(Abs_Translator):
    def __init__(self, lang: str, code: str):
        self.lang = lang
        self.code = code
        self.lang_pack = self.get_dict(lang)
        self.latest_code, self.unidentified = self.translate(code, self.lang_pack)
    ...
    def get_dict(self, lang: str)-> dict:
        ...
        if(lang.lower() == 'hindi'):
            print('hlo')
            pack_path ="convex/backend/language_packs/hindi.json"
    ...
    #Working
    def untokenize_tokens(self, token_list: list)-> str:
    ...
    def ask_user(self, token_to_verify: str, prediction: bool = False) -> str:
        if(prediction):
            ...
            if(reply == 1):
                ...
                response = self.ask_user(token_to_verify, False)
                return response
        else:
            # self.throw_error()
            response = input('Please enter the right token') 
        return response



    def map_name_token(self,token: str, lang_pack)-> str:
        changed_token = 'hlo'
        return changed_token
    


    def guess_token(self,user_token: str)-> str:
        predict = user_token
        return predict 
        ...
        for token in token_generator:
            ...
            if token_type == tokenize.NAME:
                ...
                if is_next_name:
                    ...
                    '''
                    pattern = r"\p{L}+"
                    match_result = regex.search(pattern, transliterate_string)
                    ...
                    if(keyword.iskeyword(token_string)):
                        ...
                        pred = self.guess_token(token_string)
                        '''
                        ...
                        '''
                        print("Dude its from the loop")
                        new_string = self.ask_user(pred, prediction=True)
...
print(trans.get_dict('hindi'))
print(trans.latest_code)
print(trans.unidentified)
print(trans.ask_user("hello",True))